...
export async function approveSqlApproval(approvalId: string): Promise<ApprovalResponse> {
  return requestJson<ApprovalResponse>(
    apiUrl(`/api/approvals/${encodeURIComponent(approvalId)}/approve`),
    {
      method: "POST"
    }
  );
...
export async function rejectSqlApproval(
  approvalId: string,
  reason = ""
): Promise<ApprovalResponse> {
  return requestJson<ApprovalResponse>(
    apiUrl(`/api/approvals/${encodeURIComponent(approvalId)}/reject`),
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
